...
public class PlusExpression extends NamedExpression {
    ...
    public PlusExpression(Expression left, Expression right, String name, List<String> qualifier) {
    ...
    public Field evaluate(Record record) {
        ...
        if (DataTypes.isIntegerType(dataType)) {
            ...
            return new DecimalWritable(left.getDecimal().add(right.getDecimal()));
        } else if (DataTypes.DATE == dataType) {
            TimeInterval interval = (TimeInterval) right.getObject();
            return new DateWritable(IntervalUtils.plusDate(left.getDate(), interval));
        } else if (DataTypes.TIMESTAMP == dataType) {
            TimeInterval interval = (TimeInterval) right.getObject();
            return new TimestampWritable(IntervalUtils.plusTimestamp(left.getTimestamp(), interval));
        } else {
            throw new CarbonSqlException(ErrorItem.EXPRESSION_UNSUPPORT_DATATYPE, "plusExpression", dataType.getName());
        }
    }
    ...
    protected Expression copyWithNewChildren(List<Expression> children) {
        return new PlusExpression(children.get(0), children.get(1));
    }
}
