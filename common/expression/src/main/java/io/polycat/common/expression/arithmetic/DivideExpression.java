...
public class DivideExpression extends NamedExpression {

    public DivideExpression(Expression left, Expression right) {
        this(left, right, ExpressionUtil.name(left, right, " / "), Collections.emptyList());
    }

    public DivideExpression(Expression left, Expression right, String name, List<String> qualifier) {
    ...
    public Field evaluate(Record record) {
        ...
        if (DataTypes.isIntegerType(dataType)) {
            ...
            return DecimalWritable.decimalDivide(left.getDecimal(), right.getDecimal());
        } else {
    ...
    protected Expression copyWithNewChildren(List<Expression> children) {
        return new DivideExpression(children.get(0), children.get(1));
    }
}
