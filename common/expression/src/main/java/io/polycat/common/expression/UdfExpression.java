...
public class UdfExpression extends NamedExpression {

    private int fieldIndex;

    public UdfExpression(int fieldIndex, DataType dataType) {
        super(Collections.emptyList(), dataType, null, Collections.emptyList());
        this.fieldIndex = fieldIndex;
    }

    public UdfExpression(String name) {
        super(Collections.emptyList(), null, name, Collections.emptyList());
    }

    @Override
    public String simpleString(List<Attribute> inputAttributes) {
        if (inputAttributes != null && inputAttributes.get(fieldIndex).getFieldNames().size() > 0) {
            return inputAttributes.get(fieldIndex).getFieldNames().get(0) + "=udf" + fieldIndex;
        } else {
            return "udf" + fieldIndex;
        }
    }
