...
public class TimesExpression extends NamedExpression {
    ...
    public TimesExpression(Expression left, Expression right, String name, List<String> qualifier) {
    ...
    public Field evaluate(Record record) {
        ...
        if (DataTypes.isIntegerType(dataType)) {
            ...
            return new DecimalWritable(left.getDecimal().multiply(right.getDecimal()));
        } else {
    ...
    protected Expression copyWithNewChildren(List<Expression> children) {
        return new TimesExpression(children.get(0), children.get(1));
    }
}
