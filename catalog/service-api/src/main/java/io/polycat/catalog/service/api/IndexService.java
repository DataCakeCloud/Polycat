...
// Performs Metadata operations on index
public interface IndexService {

  String createIndex(DatabaseName databaseName, IndexInput indexInput);

  void dropIndex(IndexName indexName, boolean isHMSTable);

  TraverseCursorResult<List<IndexInfo>> listIndexes(TableName tableName, boolean includeDrop,
      Integer maximumToScan, String pageToken, Boolean isHMSTable, DatabaseName databaseName);

  IndexInfo getIndexByName(IndexName indexName);

  void alterIndexByName(IndexName indexName, IndexRefreshInput indexRefreshInput);
