...
import io.polycat.catalog.common.model.PolyCatProfile;
...
@RestController
@RequestMapping("/v1/{project-id}/pcprofile")
public class PolyCatProfileController extends BaseController{
     ...
     */
    @ApiOperation(value = "get polycat profile")
    @GetMapping(value = "", produces = "application/json;charset=UTF-8")
    public CatalogResponse<PolyCatProfile> getPolyCatProfileInfo(
        ...
        return createResponse(token, () -> {
            ...
            return ResponseUtil.responseSuccess(convertPolyCatProfile(projectId, databaseList, tableList));
        });
    }
    ...
    private PolyCatProfile convertPolyCatProfile(String projectId, List<Database> databaseList, List<Table> tableList) {
        List<UserProfileContext> userProfileContexts = convertUserProfiles(databaseList, tableList);
        PolyCatProfile polyCatProfile = PolyCatProfile.builder()
            .projectId(projectId).userProfileContexts(userProfileContexts).build();
        return polyCatProfile;
    }
