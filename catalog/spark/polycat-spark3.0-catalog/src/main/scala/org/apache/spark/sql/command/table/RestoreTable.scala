...
case class RestoreTable(request: RestoreTableRequest) extends RunnableCommand with AuthSubject {
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    client.restoreTable(request)
    sparkSession.sessionState.catalogManager.currentCatalog match {
      case catalog: TableCatalog =>
        catalog.invalidateTable(Identifier.of(Array(request.getDatabaseName), request.getTableName))
      case _ =>
    }
    Seq.empty
  }
