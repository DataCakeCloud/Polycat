...
case class ShowTables(request : ListTablesRequest) extends RunnableCommand with AuthSubject{
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    client.listTables(request).getObjects.map {
      x => if(request.getIncludeDrop) {
        Row(x.getCatalogName, x.getDatabaseName, x.getTableName,
  ...
  override def output: Seq[Attribute] = {
    if (request.getIncludeDrop) {
      Seq(AttributeReference("catalog name", StringType, nullable = false)(),
        ...
        AttributeReference("drop time", StringType, nullable = true)())
    } else {
      Seq(AttributeReference("catalog name", StringType, nullable = false)(),
        AttributeReference("database name", StringType, nullable = false)(),
        AttributeReference("table name", StringType, nullable = false)()
