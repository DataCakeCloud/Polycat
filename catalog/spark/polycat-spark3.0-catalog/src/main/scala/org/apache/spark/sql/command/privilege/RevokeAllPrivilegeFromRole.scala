...
case class RevokeAllPrivilegeFromRole(request: AlterRoleRequest) extends RunnableCommand with AuthSubject{
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    client.revokePrivilegeFromRole(request)
    Seq.empty
  }
  ...
  override def getAuthorizationType = AuthorizationType.GRANT_PRIVILEGE_TO_OBJECT

  override def getOperation: Operation = request.getInput.getOperation

  override def getGrantObject: GrantObject = request.getGrantObject

  override def getCatalogObject(context: CatalogContext) = new CatalogInnerObject(request.getProjectId, null, null, null)
}
