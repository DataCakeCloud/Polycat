...
case class ShowRoles(request: ShowRolesRequest) extends RunnableCommand with AuthSubject{
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    client.showRoles(request).getObjects.map {
      x => Row(x.getCreatedTime, x.getRoleName, x.getToUsers.mkString(","), x.getOwner)
    }
  }
  override def output: Seq[Attribute] = {
    Seq(AttributeReference("created time", StringType, nullable = false)(),
      AttributeReference("role name", StringType, nullable = false)(),
      AttributeReference("to users", StringType, nullable = true)(),
      AttributeReference("owner", StringType, nullable = false)())
  }
