...
case class GrantRoleToUser(request: AlterRoleRequest) extends RunnableCommand with AuthSubject{
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    client.grantRoleToUser(request)
    Seq.empty
  }
  ...
  override def getCatalogObject(context: CatalogContext): CatalogInnerObject = {
    new CatalogInnerObject(request.getProjectId, null, null, request.getInput.getRoleName)
  }
}
