...
case class DescShare(request: GetShareRequest) extends RunnableCommand with AuthSubject{
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    val share = client.getShare(request)
    Seq(Row("Share name", share.getShareName),
      Row("Share id", share.getShareId),
      Row("Create time", share.getCreatedTime))
  }

  override def output: Seq[Attribute] = {
    Seq(AttributeReference("share description item", StringType, nullable = false)(),
      AttributeReference("share description value", StringType, nullable = false)())
  }
