...
case class GrantAllObjectPrivilegeToRole(request: AlterRoleRequest) extends RunnableCommand with AuthSubject {
  override def run(sparkSession: SparkSession): Seq[Row] = {
    val client = PolyCatClientHelper.getClientFromSession(sparkSession)
    request.initContext(client.getContext)
    client.grantAllObjectPrivilegeToRole(request)
    Seq.empty
  }
