...
case class ShowCatalogs(request: ListCatalogsRequest) extends RunnableCommand with AuthSubject {
  override def run(sparkSession: SparkSession): Seq[Row] = {
    val client = PolyCatClientHelper.getClientFromSession(sparkSession)
    request.initContext(client.getContext)
    client.listCatalogs(request).getObjects.map {
      x => Row(x.getCatalogName)
    }
  }
  override def output: Seq[Attribute] = {
    Seq(
      AttributeReference("catalog name", StringType, nullable = false)())
  }
