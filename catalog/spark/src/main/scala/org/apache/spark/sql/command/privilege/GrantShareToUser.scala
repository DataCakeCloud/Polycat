...
case class GrantShareToUser(request: AlterShareRequest) extends RunnableCommand with AuthSubject{
  override def run(sparkSession: SparkSession): Seq[Row] = {
    val client = PolyCatClientHelper.getClientFromSession(sparkSession)
    request.initContext(client.getContext)
    client.grantShareToUser(request)
    Seq.empty
  }
  ...
  override def getCatalogObject(context: CatalogContext) = new CatalogInnerObject(request.getInput.getProjectId,
    null, null, request.getInput.getShareName)

  override def getOperation = Operation.GRANT_SHARE_TO_USER
}
