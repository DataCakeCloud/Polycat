...
case class RevokeAllObjectPrivilegeFromRole(request: AlterRoleRequest) extends RunnableCommand with AuthSubject{
  override def run(sparkSession: SparkSession): Seq[Row] = {
    val client = PolyCatClientHelper.getClientFromSession(sparkSession)
    request.initContext(client.getContext)
    client.revokeAllObjectPrivilegeFromRole(request)
    Seq.empty
  }
