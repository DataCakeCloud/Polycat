...
case class RevokePrivilegeFromPrincipal(request: AlterPrivilegeRequest)  extends RunnableCommand with AuthSubject {
  override def run(sparkSession: SparkSession): Seq[Row] = {
  val client = PolyCatClientHelper.getClientFromSession(sparkSession)
  request.initContext(client.getContext)
  client.revokePrivilegeFromPrincipal(request)
  Seq.empty
  }
