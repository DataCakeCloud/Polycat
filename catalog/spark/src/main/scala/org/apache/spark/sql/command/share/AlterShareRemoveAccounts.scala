...
case class AlterShareRemoveAccounts(request : AlterShareRequest) extends RunnableCommand with AuthSubject {

  override def run(sparkSession: SparkSession): Seq[Row] = {
    val client = PolyCatClientHelper.getClientFromSession(sparkSession)
    request.initContext(client.getContext)
    client.alterShareRemoveAccounts(request)
    Seq.empty
  }
  ...
  override def getCatalogObject(context: CatalogContext) = new CatalogInnerObject(request.getProjectId, null, null, request.getInput.getShareName)
}
