...
case class DescDatabase(request: GetDatabaseRequest) extends RunnableCommand with AuthSubject{
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    Seq(Row("Catalog name", database.getCatalogName),
      Row("Database name", database.getDatabaseName),
      Row("Description", database.getDescription),
      Row("Location", database.getLocationUri),
      Row("Create time", ReadableTools.getReadableDataBySecond(database.getCreateTime)),
      Row("Owner", database.getOwner))
  }

  override def output: Seq[Attribute] = {
    Seq(AttributeReference("database description item", StringType, nullable = false)(),
      AttributeReference("database description value", StringType, nullable = false)())
  }
