...
import io.polycat.catalog.common.model.{CatalogInnerObject, GrantObject}
...
case class RevokeRoleFromUser(request: AlterRoleRequest) extends RunnableCommand with AuthSubject {
  override def run(sparkSession: SparkSession): Seq[Row] = {
    val client = PolyCatClientHelper.getClientFromSession(sparkSession)
    request.initContext(client.getContext)
    client.revokeRoleFromUser(request)
    Seq.empty
  }
  ...
  override def getOperation: Operation = request.getOperation

  override def getCatalogObject(context: CatalogContext) = new CatalogInnerObject(request.getProjectId, null, null, request.getInput.getRoleName)

  override def getGrantObject = new GrantObject(request.getProjectId, null, request.getInput.getObjectName)
}
