...
public abstract class TestHmsPolyCatBridge extends TestCase {
  private static final Logger LOG = LoggerFactory.getLogger(TestHmsPolyCatBridge.class);
  ...
  private static final String TEST_CATALOG_NAME = "test_default_catalog1";
  ...
  static {
    ...
    hiveConf.set(CatalogUserInformation.POLYCAT_USER_PROJECT, "default_project");
    hiveConf.set(CatalogUserInformation.POLYCAT_USER_TENANT, "tenant");
    ...
    silentDropCatalog(TEST_CATALOG_NAME);
    ...
    catalogInput.setCatalogName(TEST_CATALOG_NAME);
    createCatalogRequest.setInput(catalogInput);
    createCatalogRequest.setProjectId(catalogClient.getProjectId());
    createCatalogRequest.getInput().setOwner("test");
    catalogClient.createCatalog(createCatalogRequest);
  }

  private static void silentDropCatalog(String catalogName) {
    try {
      DropCatalogRequest dropCatalogRequest = new DropCatalogRequest();
      dropCatalogRequest.setCatalogName(catalogName);
      dropCatalogRequest.setProjectId(catalogClient.getProjectId());
      catalogClient.dropCatalog(dropCatalogRequest);
    } catch (Exception e) {
  ...
  protected void setUp() throws Exception {
    ...
    GlobalConfig.setItem("identity.file.path", "./../../conf/identity.conf");
  ...
  public static void partitionTester(HiveMetaStoreClient client, HiveConf hiveConf)
    ...
    try {
      ...
      boolean partition = client.dropPartition(dbName, tblName, vals6);
      assertTrue(partition);
      assertEquals(size - 1, client.listPartitions(dbName, tblName, (short) -1).size());
      ...
      assertTrue("Partitions are not the same", part.getValues().equals(part_get.getValues()));
      ...
      assertTrue("Not all parts returned", partial.stream().map(Partition::getValues).collect(Collectors.toList()).containsAll(parts.stream().map(Partition::getValues).collect(Collectors.toList())));
...
//      assertTrue("Not all part names returned", partialNames.containsAll(unescapePartitionNames(partNames)));
  ...
  private static void verifyPartitionsPublished(HiveMetaStoreClient client,
    ...
    assertTrue("Not all parts returned", mpartial.stream().map(Partition::getValues).collect(Collectors.toList()).containsAll(expectedPartitions.stream().map(Partition::getValues).collect(Collectors.toList())));
  }
  ...
  public void testListPartitionsWihtLimitEnabled() throws Exception {
    // create a table with multiple partitions
    String dbName = "compdb";
    String tblName = "comptbl";
    String typeName = "Person";
  ...
  public void testAlterViewParititon() throws Exception {
    String dbName = "compdb";
    String tblName = "comptbl";
...
//              12);
  ...
  public void testDatabase() throws Exception {
    try {
      ...
      db.setOwnerName(getSessionStateUser());
      ...
      LOG.info("getOwnerName:" + db.getOwnerName() + "    " + getSessionStateUser());
      assertEquals(db.getOwnerName(), getSessionStateUser());
  ...
  public void testDatabaseLocationWithPermissionProblems() throws Exception {
    ...
    db.setName(TEST_DB1_NAME);
    String dbLocation =
            HiveConf.getVar(hiveConf, ConfVars.METASTOREWAREHOUSE) + "/test/_testDB_create_";
    FileSystem fs = FileSystem.get(new Path(dbLocation).toUri(), hiveConf);
    fs.mkdirs(
            new Path(HiveConf.getVar(hiveConf, ConfVars.METASTOREWAREHOUSE) + "/test"),
    ...
    try {
      ...
      fs.setPermission(new Path(HiveConf.getVar(hiveConf, ConfVars.METASTOREWAREHOUSE) + "/test"),
              new FsPermission((short) 755));
      fs.delete(new Path(HiveConf.getVar(hiveConf, ConfVars.METASTOREWAREHOUSE) + "/test"), true);
    }
  ...
  public void testDatabaseLocation() throws Exception {
    try {
      ...
      db.setName(TEST_DB1_NAME);
      String dbLocation =
              HiveConf.getVar(hiveConf, ConfVars.METASTOREWAREHOUSE) + "/_testDB_create_";
      db.setLocationUri(dbLocation);
      client.createDatabase(db);
      ...
      db.setName(TEST_DB1_NAME);
      dbLocation =
              HiveConf.getVar(hiveConf, ConfVars.METASTOREWAREHOUSE) + "/_testDB_file_";
      FileSystem fs = FileSystem.get(new Path(dbLocation).toUri(), hiveConf);
  ...
  public void testSimpleTable() throws Exception {
    try {
      ...
      TestHmsPolyCatBridge.client.dropTable(dbName, tblName);
      silentDropDatabase(dbName);

      Database db = new Database();
      db.setName(dbName);
      TestHmsPolyCatBridge.client.createDatabase(db);

      TestHmsPolyCatBridge.client.dropType(typeName);
      Type typ1 = new Type();
      typ1.setName(typeName);
      typ1.setFields(new ArrayList<FieldSchema>(2));
      typ1.getFields().add(
      ...
      typ1.getFields().add(
              new FieldSchema("income", serdeConstants.INT_TYPE_NAME, ""));
      TestHmsPolyCatBridge.client.createType(typ1);
      ...
      sd.getSerdeInfo().getParameters().put(
              serdeConstants.SERIALIZATION_FORMAT, "1");
      ...
      TestHmsPolyCatBridge.client.createTable(tbl);
      ...
      if (isThriftClient) {
        ...
        // on the location being present in the 'tbl' object - so get the table
        // from the metastore
        tbl = TestHmsPolyCatBridge.client.getTable(dbName, tblName);
      }

      Table tbl2 = TestHmsPolyCatBridge.client.getTable(dbName, tblName);
      assertNotNull(tbl2);
      ...
      System.out.println("getNumBuckets: " + tbl2.getSd().getNumBuckets());
      ...
      List<FieldSchema> fieldSchemas = TestHmsPolyCatBridge.client.getFields(dbName, tblName);
      ...
      List<FieldSchema> fieldSchemasFull = TestHmsPolyCatBridge.client.getSchema(dbName, tblName);
      ...
      TestHmsPolyCatBridge.client.createTable(tbl2);
      if (isThriftClient) {
        tbl2 = TestHmsPolyCatBridge.client.getTable(tbl2.getDbName(), tbl2.getTableName());
      }

      Table tbl3 = TestHmsPolyCatBridge.client.getTable(dbName, tblName2);
      assertNotNull(tbl3);
      ...
      fieldSchemas = TestHmsPolyCatBridge.client.getFields(dbName, tblName2);
      assertNotNull(fieldSchemas);
      ...
      fieldSchemasFull = TestHmsPolyCatBridge.client.getSchema(dbName, tblName2);
      assertNotNull(fieldSchemasFull);
      ...
      tableNames.add(tblName2);
      List<Table> foundTables = TestHmsPolyCatBridge.client.getTableObjectsByName(dbName, tableNames);
      ...
      foundTables = TestHmsPolyCatBridge.client.getTableObjectsByName(dbName, tableNames);
      assertEquals(foundTables.size(), 2);

      InvalidOperationException ioe = null;
      try {
        foundTables = TestHmsPolyCatBridge.client.getTableObjectsByName(dbName, null);
      ...
      try {
        foundTables = TestHmsPolyCatBridge.client.getTableObjectsByName("db_that_doesnt_exist", tableNames);
      ...
      udbe = null;
      try {
        foundTables = TestHmsPolyCatBridge.client.getTableObjectsByName("", tableNames);
      } catch (UnknownDBException e) {
        udbe = e;
      }
      assertNotNull(udbe);
      ...
      TestHmsPolyCatBridge.client.dropTable(dbName, tblName2);

      TestHmsPolyCatBridge.client.dropType(typeName);
      TestHmsPolyCatBridge.client.dropDatabase(dbName);
    } catch (Exception e) {
      System.err.println(StringUtils.stringifyException(e));
  ...
  public void testStatsFastTrivial() throws Exception {
  ...
  public void testComplexTable() throws Exception {
    ...
    try {
      ...
      sd.getSerdeInfo().setParameters(new HashMap<String, String>());
      sd.getSerdeInfo().getParameters().put(
              serdeConstants.SERIALIZATION_FORMAT, "9");
      sd.getSerdeInfo().setSerializationLib(
              LazySimpleSerDe.class.getName());
      sd.setInputFormat(HiveInputFormat.class.getName());
      sd.setOutputFormat(HiveOutputFormat.class.getName());
  ...
  public void testTableDatabase() throws Exception {
    ...
    try {
      ...
      db.setName(dbName);
      String dbLocation =
              HiveConf.getVar(hiveConf, ConfVars.METASTOREWAREHOUSE) + "_testDB_table_create_";
      db.setLocationUri(dbLocation);
      client.createDatabase(db);
      db = client.getDatabase(dbName);
  ...
  private static void adjust(HiveMetaStoreClient client, Partition part,
    ...
    part.putToParameters(hive_metastoreConstants.DDL_TIME, Long.toString(part_get.getCreateTime()));
  }
  ...
  public void testTableFilter() throws Exception {
    try {
      ...
      filter = hive_metastoreConstants.HIVE_FILTER_FIELD_OWNER +
              " like \".*Owner.*\" and " +
              hive_metastoreConstants.HIVE_FILTER_FIELD_OWNER +
              " like  \"test.*\"";
  ...
  /*    //lastAccessTime < 90
      ...
      //test params
      //test_param_2 = "50"
      filter = hive_metastoreConstants.HIVE_FILTER_FIELD_PARAMS +
   ...
   * @throws Exception
   */
  public void ignoreTestConcurrentMetastores() throws Exception {
  ...
  private void updateTableNameInDB(String oldTableName, String newTableName) throws SQLException {
    String connectionStr = HiveConf.getVar(hiveConf, ConfVars.METASTORECONNECTURLKEY);
    int interval = 1;
    int attempts = 1;
  ...
  private Type createType(String typeName, Map<String, String> fields) throws Exception {
    Type typ1 = new Type();
    typ1.setName(typeName);
  ...
  @Test
  public void testTransactionalValidation() throws Exception {
  ...
  private List<Partition> createPartitions(String dbName, Table tbl,
                                           List<List<String>> values) throws Exception {
    int i = 1;
  ...
  private void createFunction(String dbName, String funcName, String className,
                              ...
                              FunctionType functionType, List<ResourceUri> resources)
          throws Exception {
  ...
  public void testRetriableClientWithConnLifetime() throws Exception {
    ...
    conf.setLong(ConfVars.METASTORE_CLIENT_SOCKET_LIFETIME.name(), 60);
  ...
  public void testValidateTableCols() throws Exception {
