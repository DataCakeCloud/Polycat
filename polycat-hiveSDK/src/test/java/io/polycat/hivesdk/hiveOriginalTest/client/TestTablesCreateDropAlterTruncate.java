...
public class TestTablesCreateDropAlterTruncate extends MetaStoreClientTest {
  ...
  @BeforeClass
  public  static void startUpPolyCat() throws Exception {
  ...
  public void setUp() throws Exception {
    ...
    for (String tableName : client.getAllTables(DEFAULT_DATABASE)) {
      try {
        ...
        client.dropTable(DEFAULT_DATABASE, tableName, false, true, false);
      }
    }
  ...
  public void testCreateGetDeleteTable() throws Exception {
    ...
    createdTable.setRewriteEnabled(table.isRewriteEnabled());
...
//        createdTable.getParameters().get("numFiles"));
  ...
  @Ignore("table struction not same")
  @Test
  ...
  @Ignore("not support change database")
  @Test
  ...
  @Ignore("TApplicationException not handle")
  @Test
...
//    client.updateCreationMetadata(catName, dbName, tableNames[3], cm);
  ...
  private void createCatalogIfNotExist(Catalog cat) throws TException{
    try {
      client.createCatalog(cat);
    } catch (AlreadyExistsException e) {
    }finally {
      List<String> dbs = client.getAllDatabases(cat.getName());
      for (String dbName : dbs) {
        client.dropDatabase(cat.getName(), dbName, true, true, true);
      }
    }
  }

  @Test
  @Ignore("TapplicationExecption not translate")
