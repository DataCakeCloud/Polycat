...
public class HiveDataAccessor {
    ...
    private static final HashMap<String, String> lmToHiveFileFormatMap = new HashMap() {
        {
            put("csv", "TEXTFILE");
        }
    };
...
//        binaryData.setBitVectors(DataTypeUtil.getBitVectors(polyCatBinaryData.getBitVectors()));
    ...
    public static org.apache.hadoop.hive.metastore.api.Database toDatabase(Database polyCatDatabase) {
        ...
        database.setCatalogName(polyCatDatabase.getCatalogName());
    ...
    public static PrincipalType getOwnerType(String ownerType) {
    ...
    public static Catalog toCatalog(io.polycat.catalog.common.model.Catalog catalog) {
        final Catalog hiveCatalog = new Catalog();
        hiveCatalog.setName(catalog.getCatalogName());
        hiveCatalog.setDescription(catalog.getDescription());
        hiveCatalog.setLocationUri(catalog.getLocation());
        return hiveCatalog;
    }

    public static Table toTable(io.polycat.catalog.common.model.Table lmTable) {
        if (lmTable == null) {
            return null;
        }
        Table table = new Table();
        table.setDbName(lmTable.getDatabaseName());
        table.setCatName(lmTable.getCatalogName());
        table.setTableName(lmTable.getTableName());
        table.setParameters(lmTable.getParameters());
        ...
        io.polycat.catalog.common.model.StorageDescriptor lmsTableStorageDescriptor = lmTable.getStorageDescriptor();
        ...
        List<FieldSchema> columns = lmsTableStorageDescriptor.getColumns().stream()
                .map(HiveDataAccessor::lmSchemaToMsFieldSchema).collect(Collectors.toList());
        List<FieldSchema> partitions = lmTable.getPartitionKeys().stream()
                .map(HiveDataAccessor::lmSchemaToMsFieldSchema).collect(Collectors.toList());
        ...
        table.setTableType(lmTable.getTableType());
        setTableTypeRelParam(table);

        // TODO
        if (lmTable.getPartitionKeys() != null) {
            table.setPartitionKeys(
                    lmTable.getPartitionKeys().stream()
                            .map(HiveDataAccessor::lmSchemaToMsFieldSchema).collect(Collectors.toList()));
        }
        table.setOwner(lmTable.getOwner());
        table.setViewExpandedText(lmTable.getViewExpandedText());
        table.setViewOriginalText(lmTable.getViewOriginalText());
        return table;
    }

    private static void setTableTypeRelParam(Table table) {
        if (table.getTableType().equalsIgnoreCase(TableType.MATERIALIZED_VIEW.name())) {
            if (table.getParameters().containsKey(CompatibleHiveConstants.V3_PARAM)) {
                Map<String, String> map = GsonUtil.fromJson(table.getParameters().get(CompatibleHiveConstants.V3_PARAM), Map.class);
                if (map.containsKey(CompatibleHiveConstants.V3_TABLE_CREATION_METADATA)) {
                    table.setCreationMetadata(GsonUtil.fromJson(map.get(CompatibleHiveConstants.V3_TABLE_CREATION_METADATA), CreationMetadata.class));
                }
            }
        }
    }

    private static org.apache.hadoop.hive.metastore.api.SkewedInfo convertToSkewedInfo(io.polycat.catalog.common.model.SkewedInfo lmsSkewedInfo) {
    ...
    private static FieldSchema lmSchemaToMsFieldSchema(Column schemaField) {
    ...
    private static String lmsDataTypeTohmsDataType(String lmsDataType) {
        return convertTypeMap.getOrDefault(lmsDataType, lmsDataType);
    }

    private static List<org.apache.hadoop.hive.metastore.api.Order> convertToSortOrder(List<io.polycat.catalog.common.model.Order> sortColumns) {
        if (sortColumns == null) {
    ...
    private static SerDeInfo convertToSerDeInfo(io.polycat.catalog.common.model.StorageDescriptor lmsSd) {
        io.polycat.catalog.common.model.SerDeInfo lmsSerDe = lmsSd.getSerdeInfo();
        SerDeInfo hiveSerDe = new SerDeInfo();
        if (lmsSerDe != null) {
            if (lmsSerDe.getName().isEmpty()) {
                hiveSerDe.setName(null);
            } else {
    ...
    private static String getSerde(String fileFormat) {
    ...
    private static StorageFormatDescriptor getBySourceName(String name) {
        StorageFormatFactory formatFactory = new StorageFormatFactory();
        return formatFactory.get(lmToHiveFileFormatMap.getOrDefault(name, name));
    }

    private static PrincipalType getOwnerTypeStruct(String type) {
        try {
            return PrincipalType.valueOf(type);
        } catch (Exception e) {
            return PrincipalType.valueOf("USER");
        }
    }
    ...
    public static TableMeta toTableMeta(TableBrief lsmTblMeta) {
        ...
        hiveTblMeta.setCatName(lsmTblMeta.getCatalogName());
        return hiveTblMeta;
    }
    ...
    /*public static org.apache.hadoop.hive.metastore.api.Partition toPartition(Partition partition, String catName,
        ...
        hivePartition.setCatName(catName);
        return hivePartition;
    }*/

    public static List<org.apache.hadoop.hive.metastore.api.Partition> toPartitions(List<Partition> partitions,
                          org.apache.hadoop.hive.metastore.api.StorageDescriptor sd) {
        if (partitions == null) {
            return new ArrayList<>();
        }
        return partitions.stream().map(partition ->
                    HiveDataAccessor.toPartition(partition, sd)).collect(Collectors.toList());
    }

    public static org.apache.hadoop.hive.metastore.api.Partition toPartition(Partition partition,
                                                                              org.apache.hadoop.hive.metastore.api.StorageDescriptor sd) {
        org.apache.hadoop.hive.metastore.api.Partition hivePart = new org.apache.hadoop.hive.metastore.api.Partition(
                partition.getPartitionValues(),
                partition.getDatabaseName(),
                partition.getTableName(),
                partition.getCreateTime() == null ? 0 : partition.getCreateTime().intValue(),
                partition.getLastAccessTime() == null ? 0 : partition.getLastAccessTime().intValue(),
                convertToStorageDescriptor(partition.getStorageDescriptor(), sd),
                partition.getParameters());
        hivePart.setCatName(partition.getCatalogName());
        return hivePart;
    }

    public static org.apache.hadoop.hive.metastore.api.StorageDescriptor convertToStorageDescriptor(
            io.polycat.catalog.common.model.StorageDescriptor lmsPartitionSd, org.apache.hadoop.hive.metastore.api.StorageDescriptor sd) {
        org.apache.hadoop.hive.metastore.api.StorageDescriptor storageDescriptor = new org.apache.hadoop.hive.metastore.api.StorageDescriptor();
        if (CollectionUtils.isNotEmpty(lmsPartitionSd.getColumns())) {
            storageDescriptor.setCols(lmsPartitionSd.getColumns().stream().map(HiveDataAccessor::lmsSchemaToMsFieldSchema)
                    .collect(Collectors.toList()));
        }
        ...
        storageDescriptor.setParameters(lmsPartitionSd.getParameters());
        storageDescriptor.setStoredAsSubDirectories(sd.isStoredAsSubDirectories());
    ...
    public static Map<String, String> convertToParameters(Map<String, String> serdeParameters) {
        return serdeParameters == null ? Collections.emptyMap() : serdeParameters;
    }

    public static AggrStats toAggrStats(AggrStatisticData lmStats) {
        AggrStats hiveStats = new AggrStats();
        hiveStats.setPartsFound(lmStats.getPartsFound());
        hiveStats.setColStats(lmStats.getColumnStatistics().stream().map(HiveDataAccessor::toColumnStatistics).collect(
            Collectors.toList()));
        return hiveStats;
    }
    ...
    public static org.apache.hadoop.hive.metastore.api.Function toFunction(FunctionInput funcInput) {
        List<ResourceUri> resourceUriList = new ArrayList<>();
        if (funcInput.getResourceUris() != null) {
            resourceUriList = funcInput.getResourceUris().stream().map(
                    resourceUri -> new ResourceUri(ResourceType.valueOf(resourceUri.getType().name()),
                            resourceUri.getUri())
            ).collect(Collectors.toList());
        }
        org.apache.hadoop.hive.metastore.api.Function hiveFunc = new org.apache.hadoop.hive.metastore.api.Function(
                funcInput.getFunctionName(),
                funcInput.getDatabaseName(),
                funcInput.getClassName(),
                funcInput.getOwner(),
            convertToOwnerType(funcInput.getOwnerType()),
            (int) funcInput.getCreateTime(),
            convertToFunctionType(funcInput.getFuncType()),
                resourceUriList
        );
        hiveFunc.setCatName(funcInput.getCatalogName());
        return hiveFunc;
    }
    ...
    private static PrincipalType convertToOwnerType(String ownerType) {
        return ownerType == null ? PrincipalType.USER : PrincipalType.valueOf(ownerType);
    }
    ...
    public static List<ColumnStatistics> toColumnStatisticsList(PartitionStatisticData result, String catName, String dbName,
                                                                String tableName) {
        ...
        for (Map.Entry<String, List<io.polycat.catalog.common.model.stats.ColumnStatisticsObj>> entry : statisticsResults.entrySet()) {
            if (CollectionUtils.isNotEmpty(entry.getValue())) {
                ...
                list.add(convertToColumnStatistics(statisticsObjs, false, catName, dbName, tableName, entry.getKey()));
            }
        }
        return list;
    }
