 ...
 */
package io.polycat.hivesdk.hive3;
...
public class MetaStoreTestUtils {
  ...
  Map<String, Function<Object, IMetaStoreClient>> clientFactory = new HashMap<String, Function<Object, IMetaStoreClient>>() {{
    put("polyCat", (context) -> new PolyCatMetaStoreClient());
  }};

  /**
   * Starts a MetaStore instance on get given port with the given configuration and Thrift bridge. Use it only it the
   * port is definitely free. For tests use startMetaStoreWithRetry instead so the MetaStore will find an emtpy port
   * eventually, so the different tests can be run on the same machine.
   *
  ...
  /**
   * Starts a MetaStore instance with the given configuration and given bridge. Tries to find a free port, and use it.
   * If failed tries another port so the tests will not fail if run parallel. Also adds the port to the warehouse dir,
   * so the multiple MetaStore instances will use different warehouse directories.
   *
  ...
  /**
   * Setup a configuration file for standalone mode.  There are a few config variables that have defaults that require
   * parts of Hive that aren't present in standalone mode.  This method sets them to something that will work without
   * the rest of Hive.  It only changes them if they have not already been set, to avoid clobbering intentional
   * changes.
   *
  ...
  /**
   * There is no cascade option for dropping a catalog for security reasons.  But this in inconvenient in tests, so
   * this method does it.
   *
