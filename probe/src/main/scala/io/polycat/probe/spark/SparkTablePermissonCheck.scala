...
class SparkTablePermissonCheck(sparkSession: SparkSession) extends (LogicalPlan => Unit) with Logging {
  ...
  override def apply(plan: LogicalPlan): Unit = {
    if (sparkSession.conf.get("spark.sql.authorization.enabled", "false").toBoolean) {
  ...
  def createAuthorizationInput(context: CatalogContext, catalogObject: CatalogOperationObject): AuthorizationInput = {
    ...
    authorizationInput.setOperation(catalogObject.getOperation)
